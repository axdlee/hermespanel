...
const EXTENSIONS_OVERVIEW_VIEWS: Array<{
...
}> = [
  {
    ...
    label: '常用去向',
    icon: '🚀',
    hint: '先决定去工具平台、技能来源、记忆链路还是插件进阶。',
  },
  {
    key: 'status',
    label: '当前判断',
    icon: '🌤️',
    hint: '只看扩展层最关键的状态摘要和提醒，不把平台焦点一起铺开。',
  },
  {
    key: 'focus',
    label: '当前焦点',
    icon: '🧰',
    hint: '只围绕当前平台做轻量判断，完整运行清单继续放在下一层。',
  },
];
...
const EXTENSIONS_RUNTIME_VIEWS: Array<{
...
}> = [
  {
    ...
    label: '焦点平台',
    icon: '🧰',
    hint: '默认先围绕一个平台完成核对，不把平台入口、来源筛选和列表同时铺开。',
  },
  {
    key: 'platforms',
    label: '平台入口',
    icon: '🛰️',
    hint: '需要切换平台或看平台分布时，再进入这一层。',
  },
  {
    key: 'skills',
    label: '来源观察',
    icon: '🧩',
    hint: '技能来源、trust 和筛选结果后置到独立观察层。',
  },
];
...
const EXTENSIONS_OPERATIONS_VIEWS: Array<{
...
}> = [
  {
    ...
    label: '官方入口',
    icon: '🚪',
    hint: 'Tools、Skills、Memory 和插件安装入口统一放在这里。',
  },
  {
    key: 'batch',
    label: '批量工具',
    icon: '🪄',
    hint: '一次性启停多个工具属于进阶动作，继续后置。',
  },
...
function matchingPlatformSummary(extensions: ExtensionsSnapshot, platformKey: string) {
  return extensions.toolPlatforms.find(
    item => item.name.trim().toLowerCase() === platformKey.trim().toLowerCase()
  );
}
...
export function ExtensionsPage({
  ...
  useEffect(() => {
    ...
    setRuntimeView(
      pageIntent.rawKind === 'skills'
        ? 'skills'
        : pageIntent.selectedPlatform ||
            pageIntent.toolNames?.length ||
            pageIntent.rawKind === 'tools'
          ? 'focus'
          : 'focus'
    );
    setOperationsView(
  ...
  useEffect(() => {
    ...
    if (
      extensions.toolInventory.some(item => item.platformKey === investigation.selectedPlatform)
    ) {
  ...
  const sourceOptions = useMemo(
    () => [
      'all',
      ...Array.from(new Set(runtimeSkills.map(item => item.source).filter(Boolean))).sort(),
    ],
    [runtimeSkills]
  );
  ...
  const sourceLocalCount =
    extensions.skillSourceCounts.find(item => item.name === 'local')?.count ?? 0;
  const sourceBuiltinCount =
    extensions.skillSourceCounts.find(item => item.name === 'builtin')?.count ?? 0;
  ...
  if (!installation.binaryFound) {
    warnings.push(
      '当前没有检测到 Hermes CLI，扩展安装、技能配置和 memory provider 切换都无法真正执行。'
    );
  }
  ...
  if (toollessPlatforms.length > 0) {
    warnings.push(
      `存在 ${toollessPlatforms.length} 个平台当前没有启用任何工具：${toollessPlatforms.map(item => item.name).join('、')}。`
    );
  }
  if (extensions.plugins.installedCount === 0) {
    warnings.push(
      '当前没有通过 `hermes plugins list` 检测到已安装插件，扩展层仍以 builtin 与 local skills 为主。'
    );
  }
  if (
    dashboard.config.memoryProvider &&
    dashboard.config.memoryProvider !== 'builtin-file' &&
    extensions.memoryRuntime.provider.includes('none')
  ) {
    warnings.push(
      `配置里声明了 memory provider「${dashboard.config.memoryProvider}」，但运行态显示仍是 built-in only，建议核对记忆插件是否真的生效。`
    );
  }
  if (runtimeSkillMismatch) {
    warnings.push(
      `CLI skills list 识别到 ${sourceLocalCount} 个 local 技能，而本地目录扫描到 ${skills.length} 个，说明“安装态”和“文件态”之间可能有差异。`
    );
  }
  if (dashboard.gateway?.gatewayState !== 'running' && dashboard.counts.cronJobs > 0) {
    warnings.push(
      '当前 gateway 未运行，如果 cron 作业里有远端交付目标，扩展能力没法在消息平台侧真正闭环验证。'
    );
  }
  ...
  const currentPlatformSummary = currentPlatform
    ? matchingPlatformSummary(extensions, currentPlatform.platformKey)
    : undefined;
  const currentPlatformMetrics = inventoryMetrics(currentPlatform);
  const sourceDistributionSummary =
    extensions.skillSourceCounts.map(item => `${item.name} ${item.count}`).join(' · ') || '—';
  ...
  const remainingPluginCount = Math.max(
    0,
    extensions.plugins.items.length - visiblePluginItems.length
  );
  const visibleMemoryPlugins = extensions.memoryRuntime.installedPlugins.slice(0, 4);
  const remainingMemoryPluginCount = Math.max(
    0,
    extensions.memoryRuntime.installedPlugins.length - visibleMemoryPlugins.length
  );
  const runningToolBatchEnable =
    runningAction === toolActionState('enable', currentPlatform?.platformKey ?? '', batchToolNames);
  const runningToolBatchDisable =
    runningAction ===
    toolActionState('disable', currentPlatform?.platformKey ?? '', batchToolNames);
  ...
  const visibleRuntimeTools = showAllRuntimeTools
    ? (currentPlatform?.items ?? [])
    : (currentPlatform?.items ?? []).slice(0, 4);
  ...
  const visiblePlatformSummaries = showAllPlatformSummaries
    ? prioritizedToolPlatforms
    : prioritizedToolPlatforms.slice(0, 4);
  const visibleFilteredSkills = showAllSkillResults ? filteredSkills : filteredSkills.slice(0, 6);
  const activeRuntimeView =
    EXTENSIONS_RUNTIME_VIEWS.find(item => item.key === runtimeView) ?? EXTENSIONS_RUNTIME_VIEWS[0];
  const activeOperationsView =
    EXTENSIONS_OPERATIONS_VIEWS.find(item => item.key === operationsView) ??
    EXTENSIONS_OPERATIONS_VIEWS[0];
  const activeOverviewView =
    EXTENSIONS_OVERVIEW_VIEWS.find(item => item.key === overviewView) ??
    EXTENSIONS_OVERVIEW_VIEWS[0];
  const hiddenRuntimeToolCount = Math.max(
    0,
    (currentPlatform?.items.length ?? 0) - visibleRuntimeTools.length
  );
  const hiddenPlatformSummaryCount = Math.max(
    0,
  ...
  const extensionLogsIntent = buildLogsDrilldownIntent(relaySeed, {
    ...
    contains:
      batchToolNames[0] ||
      primaryInvestigationTool(investigation) ||
      query ||
  ...
  const extensionDiagnosticsIntent = buildDiagnosticsDrilldownIntent(relaySeed, {
    ...
    suggestedCommand:
      batchToolNames.length > 0 || primaryInvestigationTool(investigation)
        ? 'tools-summary'
        : 'plugins-list',
    logName: 'agent',
  });
  const extensionConfigIntent = buildConfigDrilldownIntent(relaySeed, {
    description:
      '带着当前能力面线索回到配置中心，核对 toolsets、context engine 与 memory provider。',
      ...
      <div className="list-card" key={`${currentPlatform.platformKey}:${item.name}`}>
        <div className="list-card-title">
          ...
          <div className="pill-row">
            <Pill tone={item.enabled ? 'good' : 'warn'}>
        ...
        <Toolbar>
          <Button
            kind="primary"
            onClick={() =>
              void executeToolAction('enable', currentPlatform.platformKey, [item.name], {
                label: item.name,
              })
            }
          ...
          <Button
            kind="secondary"
            onClick={() =>
              void executeToolAction('disable', currentPlatform.platformKey, [item.name], {
                label: item.name,
              })
            }
            disabled={runningAction !== null || !item.enabled}
          ...
          <Button onClick={() => navigate('logs', toolLogsIntent)}>查看日志</Button>
        </Toolbar>
      </div>
    );
  }
  ...
  const overviewSection = (
    <>
      ...
      {overviewView === 'launch' ? (
        <Panel
          title="常用去向"
          subtitle="首页只露出真正高频的 4 个入口，其他治理动作继续收在子标签。"
        >
          <div className="workspace-shortcut-grid dashboard-launcher-grid">
            <button
              type="button"
            ...
            >
              <strong>
                <span className="dashboard-shortcut-icon">🧰</span>工具平台
              </strong>
              <span>
                {currentPlatform
                  ? `${currentPlatform.displayName} ${currentPlatformMetrics.enabled}/${currentPlatformMetrics.total}`
                  : '先选择一个工具平台'}
              </span>
            </button>
            <button
            ...
            >
              <strong>
                <span className="dashboard-shortcut-icon">🧩</span>技能来源
              </strong>
              <span>
                {runtimeSkillMismatch
                  ? '运行态和目录态存在差异'
                  : `${runtimeSkills.length} 个运行态技能已对齐`}
              </span>
            </button>
            <button
              type="button"
            ...
            >
              <strong>
                <span className="dashboard-shortcut-icon">🧠</span>记忆链路
              </strong>
              <span>
                {extensions.memoryRuntime.provider} ·{' '}
                {extensions.memoryRuntime.installedPlugins.length} 个相关插件
              </span>
            </button>
            <button
              type="button"
            ...
            >
              <strong>
                <span className="dashboard-shortcut-icon">🛠️</span>插件与进阶
              </strong>
              <span>
                {extensions.plugins.installedCount > 0
                  ? `${extensions.plugins.installedCount} 个插件与批量操作已后置`
                  : '批量开关和原始输出都已后置'}
              </span>
            </button>
          </div>
          <p className="helper-text top-gap">
            把平台、技能、记忆和插件拆成四条清晰去向，避免首页同时堆很多解释卡片。
          </p>
        </Panel>
      ) : null}
      ...
      {overviewView === 'status' ? (
        <Panel
          title="当前判断"
          subtitle="先看工具面、技能来源、记忆链路和插件层是否对齐，不把所有运行清单直接摊开。"
        >
            ...
            <div className="summary-mini-card">
              ...
              <strong className="summary-mini-value">
                {extensions.toolPlatforms.length
                  ? `${extensions.toolPlatforms.length} 个平台`
                  : '未解析'}
              </strong>
              <span className="summary-mini-meta">
                {toollessPlatforms.length === 0
                  ? '当前平台都已挂上部分工具'
                  : `无工具平台 ${toollessPlatforms.length} 个`}
              </span>
            </div>
            <div className="summary-mini-card">
              <span className="summary-mini-label">技能来源</span>
              <strong className="summary-mini-value">
                {runtimeSkillMismatch ? '安装态有差异' : '安装态已对齐'}
              </strong>
              <span className="summary-mini-meta">
                CLI local {sourceLocalCount} · 本地目录 {skills.length}
              </span>
            </div>
            <div className="summary-mini-card">
              ...
              <span className="summary-mini-meta">
                Built-in {extensions.memoryRuntime.builtInStatus} · 可用记忆插件{' '}
                {extensions.memoryRuntime.installedPlugins.length}
              </span>
            </div>
            <div className="summary-mini-card">
              ...
              <strong className="summary-mini-value">
                {extensions.plugins.installedCount > 0
                  ? `${extensions.plugins.installedCount} 个`
                  : '未安装'}
              </strong>
              <span className="summary-mini-meta">
                {extensions.plugins.installHint || '当前主要依赖 builtin 与 local skills。'}
              </span>
            </div>
          </div>

          {extensions.memoryRuntime.installedPlugins.length > 0 ||
          extensions.plugins.items.length > 0 ? (
            <div className="pill-row top-gap">
              {visibleMemoryPlugins.map(item => (
                <Pill key={`${item.name}:${item.availability}`}>
                  {item.name} · {item.availability}
                </Pill>
              ))}
              {remainingMemoryPluginCount > 0 ? (
                <Pill>{`+${remainingMemoryPluginCount} 个记忆插件`}</Pill>
              ) : null}
          ...
          {overviewWarnings.length > 0 ? (
            <>
              ...
              {remainingWarningCount > 0 ? (
                <p className="helper-text top-gap">
                  其余 {remainingWarningCount} 条提醒继续收在“工具与来源”和“插件与材料”里。
                </p>
              ) : null}
            </>
          ) : (
            <EmptyState
              title="扩展层状态清晰"
              description="当前没有发现明显的扩展层结构性问题，可以继续按平台或技能来源看细节。"
            />
          )}
      ...
      {overviewView === 'focus' ? (
        <Panel
          title="当前焦点平台"
          subtitle="只保留当前平台的核心判断，完整运行清单与批量动作继续留在下一层。"
        >
    ...
    <>
      ...
      {runtimeView === 'focus' ? (
        <Panel
          ...
          aside={
            <Toolbar>
              ...
              <Button onClick={() => void load({ silent: true })} disabled={runningAction !== null}>
                刷新运行态
              </Button>
            </Toolbar>
          }
        >
          {currentPlatform ? (
            <>
              <div className="workspace-summary-strip">
                <section className="summary-mini-card">
                  ...
                  <span className="summary-mini-meta">
                    {currentPlatform.displayName} 当前可治理 {currentPlatform.items.length} 项
                  </span>
                </section>
                <section className="summary-mini-card">
              ...
              <Toolbar className="top-gap">
                <Button kind="primary" onClick={() => navigate('logs', extensionLogsIntent)}>
                  查看日志
                </Button>
                <Button onClick={() => navigate('diagnostics', extensionDiagnosticsIntent)}>
                  进入诊断页
                </Button>
                <Button
                  onClick={() => {
                    setOperationsView('batch');
                    setActiveTab('operations');
                  }}
                >
                  批量与插件
              ...
              {currentPlatform.items.length > 0 ? (
                <>
                  <div className="list-stack top-gap">{visibleRuntimeTools.map(renderToolRow)}</div>
                  {currentPlatform.items.length > visibleRuntimeTools.length ? (
                    <Toolbar className="top-gap">
                      <Button onClick={() => setShowAllRuntimeTools(value => !value)}>
                        {showAllRuntimeTools
                          ? '收起工具清单'
                          : `展开更多工具（剩余 ${hiddenRuntimeToolCount} 项）`}
                      </Button>
                    </Toolbar>
                  ) : null}
                </>
                ...
                <EmptyState
                  title="平台下暂无工具"
                  description="当前平台没有从 `hermes tools list` 解析到可治理项。"
                />
              )}
            </>
          ) : (
            <EmptyState
              title="暂无平台清单"
              description="`hermes tools list --platform <platform>` 还没有返回可治理的平台内容。"
            />
          )}
        </Panel>
      ) : null}
      ...
      {runtimeView === 'platforms' ? (
        <Panel
          ...
          aside={
            extensions.toolPlatforms.length > 4 ? (
              <Toolbar>
                <Button onClick={() => setShowAllPlatformSummaries(value => !value)}>
                  {showAllPlatformSummaries
                    ? '收起平台'
                    : `展开更多平台（剩余 ${hiddenPlatformSummaryCount} 个）`}
                </Button>
              </Toolbar>
            ) : undefined
          }
        >
          <div className="workspace-summary-strip">
            <section className="summary-mini-card">
              ...
              <strong className="summary-mini-value">
                {currentPlatform?.displayName || '尚未选择平台'}
              </strong>
            ...
            <section className="summary-mini-card">
              ...
              <span className="summary-mini-meta">
                {toollessPlatforms[0]?.name || '当前平台都已挂上部分工具'}
              </span>
            </section>
            <section className="summary-mini-card">
              <span className="summary-mini-label">来源分布</span>
              <strong className="summary-mini-value">
                {extensions.skillSourceCounts.length} 类
              </strong>
          ...
          {visiblePlatformSummaries.length ? (
            <div className="workspace-shortcut-grid dashboard-launcher-grid top-gap">
              {visiblePlatformSummaries.map(platform => {
                const targetPlatformKey =
                  extensions.toolInventory.find(
                    item => item.platformKey === platform.name || item.displayName === platform.name
                  ...
                  >
                    <strong>
                      <span className="dashboard-shortcut-icon">
                        {platform.enabledCount > 0 ? '🟢' : '🟠'}
                      </span>
                      {platform.name}
                    </strong>
                    <span>
                      {platform.enabledCount > 0
                        ? `${platform.enabledCount}/${platform.totalCount} 已启用`
                        : '当前没有启用工具'}
                    </span>
                  </button>
                );
            ...
            <EmptyState
              title="暂无工具摘要"
              description="`hermes tools --summary` 没有成功返回可解析的平台摘要。"
            />
          )}
          {extensions.toolPlatforms.length > visiblePlatformSummaries.length ? (
            <p className="helper-text top-gap">
              其余 {hiddenPlatformSummaryCount} 个平台已先收起，避免入口区默认堆满。
            </p>
          ) : null}
          <Toolbar className="top-gap">
            <Button kind="primary" onClick={() => setRuntimeView('focus')}>
              回到焦点平台
            </Button>
            <Button onClick={() => navigate('diagnostics', extensionDiagnosticsIntent)}>
              进入诊断页
        ...
        >
          <div className="workspace-summary-strip">
            ...
            <section className="summary-mini-card">
              ...
              <strong className="summary-mini-value">
                {extensions.skillTrustCounts.length} 类
              </strong>
              <span className="summary-mini-meta">
                {extensions.skillTrustCounts
                  .map(item => `${item.name} ${item.count}`)
                  .join(' · ') || '—'}
              </span>
            </section>
            ...
            <section className="summary-mini-card">
              ...
              <span className="summary-mini-meta">
                如果要走消息平台或自动化验证，最终还要联动 Gateway、Cron 和 Logs。
              </span>
            </section>
          </div>
            ...
            <div className="list-stack top-gap">
              {visibleFilteredSkills.map(item => (
                <div className="list-card" key={`${item.name}:${item.source}:${item.trust}`}>
                  ...
                  <p>
                    {item.category || 'uncategorized'} · source {item.source} · trust {item.trust}
                  </p>
                </div>
              ))}
            </div>
          ...
          {filteredSkills.length > visibleFilteredSkills.length ? (
            <Toolbar className="top-gap">
              <Button onClick={() => setShowAllSkillResults(value => !value)}>
                {showAllSkillResults
                  ? '收起筛选结果'
                  : `展开更多结果（剩余 ${hiddenSkillResultCount} 条）`}
              </Button>
            </Toolbar>
          ) : null}
          <Toolbar className="top-gap">
            <Button kind="primary" onClick={() => navigate('skills')}>
              去技能页
            </Button>
    ...
    <>
        ...
        >
          <div className="control-card-grid">
            <section className="action-card action-card-compact">
              ...
              <Toolbar>
                <Button
                  kind="primary"
                  onClick={() =>
                    void openInTerminal('extensions:tools', '工具面配置', 'hermes tools')
                  }
                ...
                <Button
                  onClick={() =>
                    void openInTerminal(
                      ...
                      '工具选择向导',
                      installation.toolsSetupCommand
                    )
                  }
                  disabled={runningAction !== null || !installation.binaryFound}
                >
            ...
            <section className="action-card action-card-compact">
              ...
              <p className="action-card-copy">
                技能既有目录态也有安装态，优先用官方 registry / config
                入口核对，再回到技能页看本地目录。
              </p>
              <p className="command-line">
                hermes skills browse · {installation.skillsConfigCommand} · hermes skills list
              </p>
              <Toolbar>
                <Button
                  kind="primary"
                ...
                <Button
                  onClick={() =>
                    void openInTerminal(
                      ...
                      '技能开关',
                      installation.skillsConfigCommand
                    )
                  }
                  disabled={runningAction !== null || !installation.binaryFound}
                >
            ...
            <section className="action-card action-card-compact">
              ...
              <p className="action-card-copy">
                Hermes 的记忆 provider 和 context engine
                都应通过官方交互式入口切换，而不是在客户端私自重建逻辑。
              </p>
              ...
              <Toolbar>
                ...
                <Button
                  ...
                  disabled={runningAction !== null || !installation.binaryFound}
                >
                  {runningAction === 'extensions:plugins-panel'
                    ? '插件与 Provider 面板…'
                    : '插件与 Provider 面板'}
                </Button>
              </Toolbar>
            ...
            <section className="action-card action-card-compact">
              ...
              <p className="action-card-copy">
                这里直接把 plugin 生命周期命令交给 Terminal，既保留 Hermes
                原生命令，又能补上安装闭环。
              </p>
              <p className="command-line">
                {pluginTerminalName
                ...
                <Button
                  ...
                  onClick={() =>
                    void openInTerminal(
                      ...
                      '安装插件',
                      `hermes plugins install ${pluginTerminalName}`
                    )
                  }
                  disabled={
                    runningAction !== null || !installation.binaryFound || !pluginTerminalName
                  }
                >
                ...
                <Button
                  onClick={() =>
                    void openInTerminal(
                      ...
                      '更新插件',
                      `hermes plugins update ${pluginTerminalName}`
                    )
                  }
                  disabled={
                    runningAction !== null || !installation.binaryFound || !pluginTerminalName
                ...
                <Button
                  ...
                  onClick={() =>
                    void openInTerminal(
                      ...
                      '移除插件',
                      `hermes plugins remove ${pluginTerminalName}`,
                      `确定移除插件 ${pluginTerminalName || ''} 吗？`
                    )
                  }
                  disabled={
                    runningAction !== null || !installation.binaryFound || !pluginTerminalName
        ...
        >
          ...
          <Toolbar>
            <Button
              ...
              onClick={() =>
                currentPlatform &&
                void executeToolAction('enable', currentPlatform.platformKey, batchToolNames, {
                  clearInput: true,
            ...
            <Button
              onClick={() =>
                currentPlatform &&
                void executeToolAction('disable', currentPlatform.platformKey, batchToolNames, {
                  clearInput: true,
                  label: `${currentPlatform.displayName} 批量工具`,
                })
              }
          ...
          <p className="helper-text top-gap">
            这里直接调用 Hermes 原生命令 `hermes tools enable|disable --platform ...`，不重建 Hermes
            的配置结构。
          </p>
        </Panel>
      ) : null}
      ...
      {operationsView === 'plugins' ? (
        ...
        >
          ...
          <p className="helper-text">
            `plugins list` 当前主要暴露“已安装插件”，不会像 tools
            一样直接给出启用态，所以这里把命令入口和原始输出分开，避免同时把页面挤满。
          </p>
          {extensions.plugins.items.length > 0 ? (
            <div className="list-stack top-gap">
              {extensions.plugins.items.map(item => {
                ...
                return (
                  <div className="list-card" key={item}>
                    ...
                    <p>
                      插件管理器已识别该插件。是否在当前 profile 生效，请结合命令输出、记忆 provider
                      与相关功能页一起判断。
                    </p>
                    <Toolbar>
                      <Button
                      ...
                      <Button
                        onClick={() => setPluginNameInput(item)}
                        disabled={runningAction !== null}
                      >
                        填入输入框
                      </Button>
            ...
            <EmptyState
              title="尚未安装插件"
              description="当前还是 builtin + local skills 主导的扩展形态。若要扩展 memory/provider，通常需要先安装对应插件。"
            />
          )}
        </Panel>
      ) : null}
      ...
      {operationsView === 'output' ? (
        <div className="page-stack">
          <Panel
            title="最近命令回显"
            subtitle="每一次治理操作都保留 Hermes 原生命令、退出码和 stdout/stderr，方便你审计包装层行为。"
          >
            {lastCommand ? (
              <div className="result-stack">
                <div className="detail-list compact">
                  ...
                  <KeyValueRow
                    label="执行结果"
                    value={
                      <Pill tone={lastCommand.success ? 'good' : 'bad'}>
                        {String(lastCommand.success)}
                      </Pill>
                    }
                  />
                </div>
              ...
              <EmptyState
                title="尚未执行治理操作"
                description="这里会回显最近一次 tools / plugins 动作的 Hermes CLI 输出。"
              />
            )}
          </Panel>
    ...
    <div className="page-stack">
      ...
      >
        <div className="workspace-summary-strip">
          ...
          <section className="summary-mini-card">
            ...
            <span className="summary-mini-meta">
              Builtin / Local {sourceBuiltinCount} / {sourceLocalCount}
            </span>
          </section>
          <section className="summary-mini-card">
            <span className="summary-mini-label">记忆链路</span>
          ...
          <section className="summary-mini-card">
            ...
            <span className="summary-mini-meta">
              最近更新时间 {formatTimestamp(dashboard.gateway?.updatedAt)}
            </span>
          </section>
        </div>
        <div className="detail-list compact top-gap">
          ...
          <KeyValueRow
            label="当前焦点平台"
            value={currentPlatform?.displayName || '尚未选择平台'}
          />
        </div>
      </Panel>
        ...
        <ContextBanner
          ...
          meta={
            <div className="pill-row">
              ...
              {investigation.selectedPlatform ? (
                <Pill tone="warn">{investigation.selectedPlatform}</Pill>
              ) : null}
      ...
      <div className="tab-bar">
        {EXTENSIONS_TABS.map(tab => (
          ...
          >
            {tab.label}
            {tab.key === 'runtime' && (toollessPlatforms.length > 0 || runtimeSkillMismatch) ? (
              <span className="tab-dirty-dot" />
            ) : null}
            {tab.key === 'operations' && (warnings.length > 0 || lastCommand) ? (
              <span className="tab-dirty-dot" />
            ) : null}
          </button>
        ))}
