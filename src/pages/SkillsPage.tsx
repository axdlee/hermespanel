...
const OVERVIEW_VIEWS: Array<{
  key: SkillsOverviewViewKey;
  label: string;
  icon: string;
  hint: string;
}> = [
  {
    ...
    label: '常用去向',
    icon: '🚀',
    hint: '先决定去安装启用、扩展运行态、自动化编排还是本地维护。',
  },
  {
    key: 'focus',
    label: '当前焦点',
    icon: '🌤️',
    hint: '只看当前技能的一层摘要和下一步动作，不把提醒区一起铺开。',
  },
  {
    key: 'warnings',
    label: '关键提醒',
    icon: '⚠️',
    hint: '真正影响技能闭环的提醒单独收成一层，需要时再看。',
  },
];
...
const RAIL_VIEWS: Array<{ key: SkillsRailViewKey; label: string; icon: string; hint: string }> = [
  { key: 'pick', label: '选择技能', icon: '🗂️', hint: '先搜索和筛选，再从短列表锁定一个技能。' },
  {
    ...
    label: '当前焦点',
    icon: '🎯',
    hint: '只看当前技能摘要和快捷动作，不和长列表同屏。',
  },
];

const REGISTRY_VIEWS: Array<{
  key: SkillsRegistryViewKey;
  label: string;
  icon: string;
  hint: string;
}> = [
  {
    ...
    label: '搜索与安装',
    icon: '📦',
    hint: '默认先做搜索、预检和安装，不把低频治理动作堆在首页。',
  },
  {
    key: 'governance',
    label: '更新与核对',
    icon: '🧭',
    hint: '更新、审计和配置链路核对统一收进这一层。',
  },
  {
    key: 'wiring',
    label: '目标布线',
    icon: '🔗',
    hint: '只在需要确认目录、运行态和编排是否闭环时再看。',
  },
  {
    key: 'output',
    label: '最近回执',
    icon: '🧾',
    hint: '原始输出后置到单独一层，避免主视图被日志占满。',
  },
];
...
const STUDIO_VIEWS: Array<{ key: SkillsStudioViewKey; label: string; icon: string; hint: string }> =
  [
    ...
    { key: 'content', label: 'SKILL.md', icon: '📝', hint: '正文编修和文件定位只在这一层出现。' },
    {
      ...
      label: '目录控制',
      icon: '🧯',
      hint: '危险操作与目录定位继续后置，避免误触。',
    },
    {
      key: 'sources',
      label: '导入与新建',
      icon: '🧰',
      hint: '导入现成技能或新建私有技能时，再进入这里。',
    },
  ];
...
export function SkillsPage({ notify, profile, navigate }: PageProps) {
  ...
  const [skillFrontmatterDraft, setSkillFrontmatterDraft] = useState<SkillFrontmatterDraft | null>(
    null
  );
  ...
  async function load() {
    setLoading(true);
    try {
      const [nextSkills, nextSnapshot, nextInstallation, nextExtensions, nextCron] =
        await Promise.all([
  ...
  async function executeSkillAction(
    ...
    try {
      ...
      notify(
        ...
        result.success ? `${label} 已在客户端执行。` : `${label} 执行失败，请查看动作回执。`
      );
      if (options?.refresh) {
        await load();
      }
  ...
  async function deleteLocalSkill(
    selectedSkill: SkillItem | null,
    selectedJobs: CronJobsSnapshot['jobs']
  ) {
    if (!selectedSkill) {
  ...
  const categories = useMemo(
    () => [
      'all',
      ...Array.from(new Set(skills.map(skill => skill.category).filter(Boolean))).sort(),
    ],
    [skills]
  );
  ...
  const workspaceTabLabel =
    WORKSPACE_TABS.find(tab => tab.key === workspaceTab)?.label ?? '常用总览';
  const activeOverviewView =
    OVERVIEW_VIEWS.find(item => item.key === overviewView) ?? OVERVIEW_VIEWS[0];
  const activeRailView = RAIL_VIEWS.find(item => item.key === railView) ?? RAIL_VIEWS[0];
  const activeRegistryView =
    REGISTRY_VIEWS.find(item => item.key === registryView) ?? REGISTRY_VIEWS[0];
  ...
  const warnings = useMemo(() => {
    ...
    if (
      ...
      jobs.some(job => job.deliver !== 'local' && job.deliver !== 'origin')
    ) {
    ...
    if (extensions && runtimeMismatch) {
      nextWarnings.push(
        `运行态本地技能为 ${runtimeLocalSkills} 个，本地目录扫描为 ${skills.length} 个。`
      );
    }
    ...
    runtimeMismatch,
    selectedSkill,
    skillUsageJobs.length,
    skills.length,
    snapshot?.config.memoryEnabled,
    snapshot?.gateway?.gatewayState,
    usedSkillNames,
  ]);
  ...
  const frontmatterDirty = Boolean(
    ...
    (skillFrontmatterDraft.name.trim() !== (skillFile?.name || selectedSkill.name || '').trim() ||
      skillFrontmatterDraft.description.trim() !==
        (skillFile?.description || selectedSkill.description || '').trim())
  );
  ...
  const extensionsIntent = buildExtensionsDrilldownIntent(relaySeed, {
    ...
    sourceFilter: (extensions?.runtimeSkills ?? []).some(item => item.source === 'local')
      ? 'local'
      : 'all',
  });
    ...
    return (
      <section className="workspace-main-card">
        <div className="workspace-main-header">
          ...
          <Pill tone={lastCommand ? 'good' : 'neutral'}>
            {lastCommand ? '最近回执' : '底层快照'}
          </Pill>
        </div>
        {lastCommand ? (
          <>
            ...
            <pre className="code-block compact-code top-gap">
              {lastCommand.stdout || lastCommand.stderr || '无输出'}
            </pre>
          </>
        ) : (
          <EmptyState
            title="尚未执行动作"
            description="先在“安装与启用”或“本地维护”里执行一次动作，这里会保留最近结果。"
          />
        )}
        <pre className="code-block compact-code top-gap">
          {extensions?.skillsRawOutput || '当前没有可展示的技能原始输出。'}
        </pre>
      </section>
    );
  }
  ...
  return (
    <div className="page-stack">
      <Panel
        ...
        tip={
          <InfoTip content="这里不再堆大段说明。核心只保留技能目录、安装动作、运行态差异和自动化引用，跨页入口尽量收敛到少量真正有用的跳转。" />
        }
        aside={
          <Toolbar>
        ...
        <div className="workspace-summary-strip workspace-summary-strip-dense">
          <section className="summary-mini-card">
            ...
            <span className="summary-mini-meta">
              当前实例扫描到的本地技能，共 {uniqueCategories} 个分类
            </span>
          </section>
          <section className="summary-mini-card">
            ...
            <span className="summary-mini-meta">
              {runtimeMismatch ? '运行面与目录存在偏差' : '运行面与目录已对齐'}
            </span>
          </section>
          <section className="summary-mini-card">
            <span className="summary-mini-label">当前目标</span>
            <strong className="summary-mini-value">
              {installTarget.trim() || selectedSkill?.name || '未指定'}
            </strong>
            <span className="summary-mini-meta">
              {registryQuery.trim() || '优先在这里搜索、预检、安装技能'}
            </span>
          </section>
          <section className="summary-mini-card">
      ...
      <Panel
        ...
        tip={
          <InfoTip content="参考 ClawPanel 的轻卡片工作台，把真正高频的动作压进主区，把危险操作和低频编辑收进 disclosure。" />
        }
        aside={
          <Toolbar>
        ...
        <div className="tab-bar tab-bar-dense">
          {WORKSPACE_TABS.map(tab => (
            ...
            >
              {tab.label}
              {tab.key === 'registry' &&
              (!currentToolsets.length || runtimeMismatch || Boolean(lastCommand)) ? (
                <span className="tab-dirty-dot" />
              ) : null}
              {tab.key === 'studio' &&
              (frontmatterDirty || skillFileDirty || Boolean(lastImportedSkill)) ? (
                <span className="tab-dirty-dot" />
              ) : null}
            </button>
          ))}
            ...
            <div className="workspace-rail-header">
              <div>
                ...
                <p className="config-section-desc">
                  左侧也拆成二级工作面，避免搜索、焦点和长列表同时堆叠。
                </p>
              </div>
              <Pill tone={selectedSkill ? 'neutral' : 'warn'}>
                {selectedSkill?.name || '等待选择'}
              </Pill>
            </div>
                    ...
                    <>
                      <div className="list-stack workspace-list-scroll top-gap">
                        {visibleFilteredSkills.map(skill => {
                          ...
                          const runtimeVisible = extensions.runtimeSkills.some(
                            ...
                            >
                              <div className="list-card-title">
                                ...
                                <div className="pill-row">
                                  ...
                                  <Pill tone={runtimeVisible ? 'good' : 'warn'}>
                                    {runtimeVisible ? '运行面可见' : '待进入运行面'}
                                  </Pill>
                                </div>
                              </div>
                              <p className="skill-list-copy">
                                {truncate(
                                  skill.description || skill.preview || '底层未返回描述。',
                                  72
                                )}
                              </p>
                      ...
                      {filtered.length > visibleFilteredSkills.length ? (
                        <div className="workspace-rail-toolbar top-gap">
                          <Button onClick={() => setShowAllFilteredSkills(current => !current)}>
                            {showAllFilteredSkills
                              ? '收起技能列表'
                              : `展开更多技能（剩余 ${hiddenFilteredSkillCount} 项）`}
                          </Button>
                        </div>
                      ) : null}
                    </>
              ...
              <>
                <section className="workspace-rail-section">
                  <div className="workspace-rail-section-header">
                    ...
                    <Pill
                      tone={selectedSkill ? (selectedExistsInRuntime ? 'good' : 'warn') : 'neutral'}
                    >
                      {selectedSkill
                      ...
                      <div className="workspace-rail-toolbar workspace-rail-toolbar-grid top-gap">
                        ...
                        <Button
                          ...
                          onClick={() =>
                            selectedSkill &&
                            void openInFinder(
                              directoryOf(selectedSkill.filePath),
                              `${selectedSkill.name} 技能目录`
                            )
                          }
                        >
                          打开目录
                        </Button>
                        <Button onClick={() => setRailView('pick')}>回到列表</Button>
                      </div>
                      <p className="helper-text top-gap">
                        默认只回答“当前是谁、状态怎样、下一步去哪”，其他细节继续留在右侧主区。
                      </p>
                    </>
                  ) : (
                    <div className="top-gap">
                      <EmptyState
                        title="还没选中技能"
                        description="先切回“选择技能”，再从短列表点一个目标。"
                      />
                    </div>
                  )}
                </section>
          ...
          <div className="workspace-main">
                  ...
                  <section className="workspace-main-card top-gap">
                    <div className="workspace-main-header">
                      <div>
                        ...
                        <p className="config-section-desc">
                          首页只保留最常用的四条路径，安装、运行态、编排和本地维护分开露出。
                        </p>
                      </div>
                    </div>
                    <div className="workspace-shortcut-grid dashboard-launcher-grid">
                      ...
                      >
                        <strong>
                          <span className="dashboard-shortcut-icon">📦</span>安装与启用
                        </strong>
                        <span>
                          {selectedSkill
                            ? `${selectedSkill.name} ${selectedExistsInRuntime ? '可继续核对安装态' : '建议先做启用或预检'}`
                            : '先从左侧选择一个技能'}
                        </span>
                      </button>
                      <button
                      ...
                      >
                        <strong>
                          <span className="dashboard-shortcut-icon">🧩</span>运行态核对
                        </strong>
                        <span>
                          {selectedSkill
                            ? selectedExistsInRuntime
                      ...
                      >
                        <strong>
                          <span className="dashboard-shortcut-icon">⏱️</span>自动化编排
                        </strong>
                        <span>
                          {selectedSkill
                            ? `${skillUsageJobs.length} 个作业显式引用当前技能`
                            : `${jobs.length} 个作业可继续核对技能布线`}
                        </span>
                      </button>
                      <button
                        type="button"
                      ...
                      >
                        <strong>
                          <span className="dashboard-shortcut-icon">📝</span>本地维护
                        </strong>
                        <span>
                          {selectedSkill
                            ? skillFile
                              ? 'frontmatter 与正文已经可编辑'
                              : '正文正在准备接管'
                            : '需要时再进入本地维护'}
                        </span>
                      </button>
                    ...
                    <p className="helper-text top-gap">
                      常用总览只负责决定下一步去哪里，不再直接铺开大量技能细节与编辑块。
                    </p>
                  </section>
                ) : null}
                  ...
                  <section className="workspace-main-card top-gap">
                    <div className="workspace-main-header">
                      <div>
                        ...
                        <p className="config-section-desc">
                          默认只保留一层摘要和少量后续动作，避免总览页继续变成信息墙。
                        </p>
                      </div>
                      <Pill
                        tone={
                          selectedSkill ? (selectedExistsInRuntime ? 'good' : 'warn') : 'neutral'
                        }
                      >
                        {selectedSkill
                          ? selectedExistsInRuntime
                      ...
                      <>
                        <div className="workspace-summary-strip">
                          ...
                          <section className="summary-mini-card">
                            ...
                            <strong className="summary-mini-value">
                              {selectedExistsInRuntime ? '已接入' : '待同步'}
                            </strong>
                            <span className="summary-mini-meta">
                              {selectedExistsInRuntime
                                ? '当前实例已能看到它'
                          ...
                          <section className="summary-mini-card">
                            ...
                            <span className="summary-mini-meta">
                              {skillFile ? '正文已接管' : '正文待读取'}
                            </span>
                          </section>
                        </div>
                        <p className="helper-text top-gap">
                          {selectedSkill.preview || '当前技能没有更多预览内容。'}
                        </p>
                    ...
                    {overviewWarnings.length > 0 ? (
                      <>
                        ...
                        {remainingWarningCount > 0 ? (
                          <p className="helper-text top-gap">
                            其余 {remainingWarningCount}{' '}
                            条提醒继续收在安装与启用、本地维护等子模块里。
                          </p>
                        ) : null}
                      </>
                    ) : (
                      <EmptyState
                        title="当前提醒不多"
                        description="技能层暂时没有新的结构性风险，可以继续去安装启用或消息日志做下一步核对。"
                      />
                    )}
                  </section>
                ) : null}
            ...
            {workspaceTab === 'registry' ? (
              <>
                <section className="workspace-main-card">
                  <div className="workspace-main-header">
                    <div>
                      ...
                      <p className="config-section-desc">
                        这里只先决定当前要做哪类动作，具体内容一次只展开一层。
                      </p>
                    </div>
                    <Pill tone={installTarget.trim() ? 'good' : 'neutral'}>
                      {installTarget.trim() || selectedSkill?.name || '等待目标'}
                    </Pill>
                  </div>
                  <div className="workspace-shortcut-grid dashboard-launcher-grid">
                  ...
                  <section className="workspace-main-card top-gap">
                    <div className="workspace-main-header">
                      <div>
                        ...
                        <p className="config-section-desc">
                          先搜关键词，再对目标技能做预检或安装，这是最常走的技能接入路径。
                        </p>
                      </div>
                      <Pill tone={installTarget.trim() ? 'good' : 'warn'}>
                        {installTarget.trim() || '等待目标'}
                      </Pill>
                    </div>
                    ...
                    <Toolbar className="top-gap">
                      <Button
                        ...
                        onClick={() =>
                          void executeSkillAction('search', registryQuery.trim(), {
                      ...
                      <Button
                        ...
                        onClick={() =>
                          void executeSkillAction('inspect', installTarget.trim(), {
                      ...
                      <Button
                        disabled={runningAction !== null || !installTarget.trim()}
                        onClick={() =>
                          void executeSkillAction('install', installTarget.trim(), {
                      ...
                      <Button
                        disabled={runningAction !== null || !selectedSkill}
                        onClick={() => selectedSkill && openRegistryInstall(selectedSkill)}
                      >
                        使用当前技能
                      </Button>
                    </Toolbar>
                    <p className="helper-text top-gap">
                      调用的是桌面端封装动作，不会把你甩回外部终端。
                    </p>
                  </section>
                ) : null}
                  ...
                  <section className="workspace-main-card top-gap">
                    <div className="workspace-main-header">
                      <div>
                        ...
                        <p className="config-section-desc">
                          更新、审计和配置链路核查继续留在这里，不再和常用安装动作混在一起。
                        </p>
                      </div>
                    ...
                    <Toolbar className="top-gap">
                      <Button
                        ...
                        disabled={runningAction !== null}
                        onClick={() =>
                          void executeSkillAction('check', null, {
                      ...
                      <Button
                        disabled={runningAction !== null}
                        onClick={() =>
                          void executeSkillAction('update', null, {
                      ...
                      <Button
                        disabled={runningAction !== null}
                        onClick={() =>
                          void executeSkillAction('audit', null, {
                  ...
                  <section className="workspace-main-card top-gap">
                    ...
                    <div className="workspace-summary-strip">
                      <section className="summary-mini-card">
                        ...
                        <strong className="summary-mini-value">{selectedSkill?.name || '—'}</strong>
                        <span className="summary-mini-meta">
                          {selectedSkill ? categoryLabel(selectedSkill.category) : '先在左侧选技能'}
                        </span>
                      </section>
                      <section className="summary-mini-card">
                        <span className="summary-mini-label">运行态</span>
                        <strong className="summary-mini-value">
                          {selectedSkill ? (selectedExistsInRuntime ? '已进入' : '待同步') : '—'}
                        </strong>
                        <span className="summary-mini-meta">
                          {selectedSkill
                            ? selectedExistsInRuntime
                      ...
                      <section className="summary-mini-card">
                        ...
                        <strong className="summary-mini-value">
                          {selectedSkill ? skillUsageJobs.length : 0}
                        </strong>
                        <span className="summary-mini-meta">
                          {skillUsageJobs.length > 0
                            ? skillUsageJobs
                ...
                {registryView === 'output' ? (
                  <div className="top-gap">{renderResultCard()}</div>
                ) : null}
              </>
            ) : null}
              ...
              <>
                <section className="workspace-main-card">
                  <div className="workspace-main-header">
                    <div>
                      ...
                      <p className="config-section-desc">
                        先选一个维护目标，再进入对应子模块；默认不把所有编辑器和危险区同时摊开。
                      </p>
                    </div>
                    <Pill tone={selectedSkill ? 'good' : 'warn'}>
                      {selectedSkill?.name || '等待目标'}
                    </Pill>
                  </div>
                  <div className="workspace-shortcut-grid dashboard-launcher-grid">
                ...
                {!selectedSkill && studioView !== 'sources' ? (
                  <section className="workspace-main-card top-gap">
                    <EmptyState
                      title="还没选治理目标"
                      description="先从左侧技能目录选择一个 skill，本地治理区才会接管基本信息、正文和目录控制。"
                    />
                  </section>
                ) : null}
                  ...
                  <section className="workspace-main-card top-gap">
                    <div className="workspace-main-header">
                      <div>
                        ...
                        <p className="config-section-desc">
                          只接管 <code>name / description</code>，其他 frontmatter 字段保持原样。
                        </p>
                      </div>
                      <div className="pill-row">
                        <Pill tone={frontmatterDirty ? 'warn' : 'good'}>
                          {frontmatterDirty ? '未保存' : '已同步'}
                        </Pill>
                    ...
                    {skillFileLoading ? (
                      <EmptyState
                        title="正在读取技能 frontmatter"
                        description="稍后会把技能名称和描述直接填进表单。"
                      />
                    ) : !skillFrontmatterDraft ? (
                      <EmptyState
                        title="等待技能文件"
                        description="先读取当前技能文件，再继续做结构化接管。"
                      />
                    ) : (
                      <>
                        <div className="detail-list compact">
                          ...
                          <KeyValueRow
                            label="技能文件"
                            value={skillFile?.filePath || selectedSkill.filePath}
                          />
                        </div>
                        <div className="form-grid form-grid-compact top-gap">
                          <label className="field-stack">
                            ...
                            <input
                              ...
                              disabled={runningAction !== null}
                              onChange={event =>
                                setSkillFrontmatterDraft(current =>
                        ...
                        <Toolbar className="top-gap">
                          ...
                          >
                            {runningAction === 'skills:save-frontmatter'
                              ? '保存中…'
                              : '保存 frontmatter'}
                          </Button>
                          <Button
                  ...
                  <section className="workspace-main-card top-gap">
                    <div className="workspace-main-header">
                      <div>
                        ...
                        <p className="config-section-desc">
                          直接维护本地 <code>SKILL.md</code> 正文，不再跳外部终端。
                        </p>
                      </div>
                      <div className="pill-row">
                        <Pill tone={skillFileDirty ? 'warn' : 'good'}>
                          {skillFileDirty ? '未保存' : '已同步'}
                        </Pill>
                        <Pill tone={skillFileLoading ? 'warn' : 'neutral'}>
                          {skillFileLoading ? '读取中' : '正文'}
                        </Pill>
                      </div>
                    </div>
                    <div className="detail-list compact">
                ...
                {studioView === 'localops' && selectedSkill ? (
                  <section className="workspace-main-card top-gap">
                    ...
                    <div className="detail-list compact">
                      <KeyValueRow
                        label="技能目录"
                        value={directoryOf(skillFile?.filePath || selectedSkill.filePath) || '—'}
                      />
                      <KeyValueRow label="相对路径" value={selectedSkill.relativePath || '—'} />
                      <KeyValueRow
                        label="技能文件"
                      ...
                      <KeyValueRow
                        label="运行态"
                        value={selectedExistsInRuntime ? '已进入运行面' : '仅目录可见'}
                      />
                    </div>
                    ...
                    <Toolbar className="top-gap">
                      ...
                      <Button
                        ...
                        disabled={
                          runningAction !== null ||
                          skillUsageJobs.length > 0 ||
                          skillDeleteConfirm.trim() !== selectedSkill.name
                        }
                    ...
                    <section className="workspace-main-card">
                      <div className="workspace-main-header">
                        <div>
                          ...
                          <p className="config-section-desc">
                            需要接入现有技能目录或单个 <code>SKILL.md</code> 时，再进入这一层。
                          </p>
                        </div>
                        <div className="pill-row">
                          <Pill tone={lastImportedSkill ? 'good' : 'neutral'}>
                            {lastImportedSkill
                              ? `${lastImportedSkill.copiedFiles} 个文件`
                              : '等待导入'}
                          </Pill>
                      ...
                      {lastImportedSkill ? (
                        <div className="detail-list compact top-gap">
                          ...
                          <KeyValueRow
                            label="归档分类"
                            value={lastImportedSkill.imported.category}
                          />
                        </div>
                      ) : null}
                      <Toolbar className="top-gap">
                        ...
                        <Button
                          ...
                          onClick={() =>
                            ...
                            setImportDraft(current => ({
                              ...current,
                              category: selectedSkill.category || current.category,
                            }))
                          }
                        >
                          带入当前分类
                        ...
                        <Button
                          disabled={runningAction !== null}
                          onClick={() =>
                            snapshot && void openInFinder(snapshot.hermesHome, 'Hermes Home')
                          }
                        >
                          打开 Home
                        </Button>
                    ...
                    <section className="workspace-main-card">
                      <div className="workspace-main-header">
                        <div>
                          ...
                          <p className="config-section-desc">
                            只有在需要定制私有能力时，再展开创建脚手架。
                          </p>
                        </div>
                        <div className="pill-row">
                          <Pill>{createDraft.category || 'custom'}</Pill>
                          <Pill tone={createDraft.name.trim() ? 'good' : 'neutral'}>
                            {createDraft.name.trim() ? '草稿已填写' : '空白草稿'}
                          </Pill>
                        </div>
                      </div>
                      <div className="form-grid">
