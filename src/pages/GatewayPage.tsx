...
const GATEWAY_DELIVERY_VIEWS: Array<{
...
}> = [
  {
    ...
    label: '平台状态',
    icon: '🛰️',
    hint: '默认先看平台连接和异常对象，不把平台和作业两组列表同时摊开。',
  },
  {
    key: 'jobs',
    label: '远端作业',
    icon: '📮',
    hint: '只在需要核对远端 delivery 时，再看作业摘要和失败对象。',
  },
  {
    key: 'routing',
    label: '继续排查',
    icon: '🔎',
    hint: '日志、诊断和跨页联动统一放在这里，不再和列表混在一起。',
  },
];
...
const GATEWAY_REPAIR_VIEWS: Array<{
...
}> = [
  {
    ...
    label: '服务接管',
    icon: '🛠️',
    hint: 'Service 安装、启停和基础运行控制集中在这里。',
  },
  {
    key: 'materials',
    label: '链路材料',
    icon: '🗂️',
    hint: '状态文件、日志目录和上下游核对继续后置到这一层。',
  },
  {
    key: 'diagnostics',
    label: '诊断命令',
    icon: '🧭',
    hint: 'CLI 诊断命令单独放一层，避免默认占满修复页。',
  },
...
export function GatewayPage({
  ...
  async function runAction(action: 'start' | 'restart' | 'stop') {
    ...
    try {
      ...
      setLastKind(null);
      setLastActionLabel(
        action === 'start' ? '启动 Gateway' : action === 'restart' ? '重启 Gateway' : '停止 Gateway'
      );
  ...
  useEffect(() => {
    ...
    void Promise.all([load(), runDiagnostic('gateway-status', { silent: true, refresh: false })]);
  }, [profile]);
  ...
  const gatewayLogName =
    unhealthyPlatforms.length > 0 || failingRemoteJobs.length > 0 ? 'gateway.error' : 'gateway';
  const gatewayContains =
    investigation?.platformName ||
  ...
  const diagnosticsIntent = buildDiagnosticsDrilldownIntent(relaySeed, {
    ...
    suggestedCommand:
      remoteJobs.length > 0 || unhealthyPlatforms.length > 0
        ? 'gateway-status-deep'
        : 'gateway-status',
    logName: gatewayLogName,
  });
  ...
  if (!installation.binaryFound) {
    warnings.push(
      '当前还没有检测到 Hermes CLI，先完成安装后，网关 service 和平台配置才能真正闭环。'
    );
  }
  ...
  if (unhealthyPlatforms.length > 0) {
    warnings.push(
      `共有 ${unhealthyPlatforms.length} 个平台连接异常或未就绪：${unhealthyPlatforms.map(item => item.name).join('、')}。`
    );
  }
  if (failingRemoteJobs.length > 0) {
    warnings.push(
      `远端投递作业里已有 ${failingRemoteJobs.length} 条出现错误或交付异常，建议回到 Cron 和 Logs 进一步排查。`
    );
  }
  ...
  const visiblePlatforms = showAllPlatforms
    ...
    : prioritizedPlatforms.slice(0, 4);
  const visibleRemoteJobs = showAllRemoteJobs
    ? prioritizedRemoteJobs
    : prioritizedRemoteJobs.slice(0, 4);
  ...
  const activeDeliveryView =
    GATEWAY_DELIVERY_VIEWS.find(item => item.key === deliveryView) ?? GATEWAY_DELIVERY_VIEWS[0];
  const activeRepairView =
    GATEWAY_REPAIR_VIEWS.find(item => item.key === repairView) ?? GATEWAY_REPAIR_VIEWS[0];
    ...
    <>
      ...
      >
        <div className="workspace-shortcut-grid dashboard-launcher-grid">
          ...
          >
            <strong>
              <span className="dashboard-shortcut-icon">📡</span>服务与平台
            </strong>
            <span>
              {gatewayRunning
                ? `${connectedPlatforms}/${platforms.length || 0} 个平台已连通`
                : '先启动 Gateway 再看平台链路'}
            </span>
          </button>
          <button
          ...
          >
            <strong>
              <span className="dashboard-shortcut-icon">🧾</span>日志排查
            </strong>
            <span>
              {gatewayContains ? `优先筛 ${gatewayContains}` : '直接进入网关日志继续排查'}
            </span>
          </button>
          <button
            type="button"
          ...
          >
            <strong>
              <span className="dashboard-shortcut-icon">📤</span>远端作业
            </strong>
            <span>
              {remoteJobs.length > 0 ? `${remoteJobs.length} 个作业待核对` : '当前主要是本地链路'}
            </span>
          </button>
          <button
            type="button"
          ...
          >
            <strong>
              <span className="dashboard-shortcut-icon">🛠️</span>深度修复
            </strong>
            <span>
              {warnings.length > 0
                ? `${warnings.length} 条提醒已后置到修复层`
                : 'service 接管和原始输出都已后置'}
            </span>
          </button>
        </div>
        <p className="helper-text top-gap">
          默认先完成启动、平台、日志和作业这四步，不再把多张解释卡片同时铺在首屏。
        </p>
      </Panel>
      ...
      >
        <div className="workspace-summary-strip">
          ...
          <section className="summary-mini-card">
            ...
            <strong className="summary-mini-value">
              {platforms.length ? `${connectedPlatforms}/${platforms.length}` : '暂无平台'}
            </strong>
            <span className="summary-mini-meta">
              {unhealthyPlatforms.length === 0
                ? '当前没有明显平台异常。'
                : `异常平台 ${unhealthyPlatforms.length} 个`}
            </span>
          </section>
          <section className="summary-mini-card">
            <span className="summary-mini-label">远端交付</span>
            <strong className="summary-mini-value">
              {remoteJobs.length ? `${remoteJobs.length} 个作业` : '本地优先'}
            </strong>
            <span className="summary-mini-meta">
              {failingRemoteJobs.length === 0
                ? '当前没有明显交付失败。'
                : `${failingRemoteJobs.length} 个作业异常`}
            </span>
          </section>
          <section className="summary-mini-card">
            <span className="summary-mini-label">上游配置</span>
            <strong className="summary-mini-value">
              {data.config.contextEngine || 'context 未配置'}
            </strong>
        ...
        <div className="detail-list compact top-gap">
          ...
          <KeyValueRow
            label="最近诊断"
            value={
              diagnostic ? (diagnostic.success ? '最近结果成功' : '最近结果失败') : '暂未执行诊断'
            }
          />
        </div>
        {overviewWarnings.length > 0 ? (
          <>
            ...
            {remainingWarningCount > 0 ? (
              <p className="helper-text top-gap">
                其余 {remainingWarningCount} 条提醒继续收在“平台与远端”和“修复与材料”里。
              </p>
            ) : null}
          </>
        ) : (
          <EmptyState
            title="网关状态稳定"
            description="当前没有看到明显的网关结构性风险，可以继续看平台细节或远端作业映射。"
          />
        )}
        <Toolbar>
          <Button
            kind="primary"
            onClick={() => void runAction(gatewayRunning ? 'restart' : 'start')}
            disabled={runningCommand !== null || !installation.binaryFound}
          >
            {gatewayRunning ? '重启 Gateway' : '启动 Gateway'}
          </Button>
          <Button
            onClick={() => void runDiagnostic('gateway-status')}
            disabled={runningCommand !== null || !installation.binaryFound}
          >
            网关状态
          </Button>
  ...
  const deliverySection = (
    <>
      <Panel
        title="平台与远端入口"
        subtitle="平台状态、远端作业和继续排查拆成独立子视图，默认只展开一个工作面。"
      >
        <div className="workspace-shortcut-grid dashboard-launcher-grid">
        ...
        >
          <div className="workspace-summary-strip">
            <section className="summary-mini-card">
              ...
              <span className="summary-mini-meta">
                {platforms.length > 0 ? `${connectedPlatforms} 个已连通` : '尚未解析出平台'}
              </span>
            </section>
            <section className="summary-mini-card">
              ...
              <span className="summary-mini-meta">
                {unhealthyPlatforms[0]?.name || '当前没有明显异常'}
              </span>
            </section>
            <section className="summary-mini-card">
              <span className="summary-mini-label">状态文件</span>
              <strong className="summary-mini-value">
                {installation.gatewayStateExists ? '已就绪' : '缺失'}
              </strong>
              <span className="summary-mini-meta">
                {installation.gatewayStateExists
                  ? 'service 状态已可回收'
                  : '建议先启动一次 Gateway'}
              </span>
            </section>
          ...
          {visiblePlatforms.length ? (
              ...
              {visiblePlatforms.map(platform => (
                <button
                  ...
                  onClick={() =>
                    navigate(
                      'logs',
                      buildLogsDrilldownIntent(relaySeed, {
                        description: `查看平台 ${platform.name} 关联的网关日志。`,
                        logName:
                          platformTone(platform.state) === 'good' ? 'gateway' : 'gateway.error',
                ...
                >
                  <strong>
                    <span className="dashboard-shortcut-icon">
                      {platformTone(platform.state) === 'good' ? '🟢' : '🟠'}
                    </span>
                    {platform.name}
                  </strong>
                  <span>
                    {platform.errorMessage
                      ? truncate(platform.errorMessage, 56)
                      : `${platform.state} · ${formatTimestamp(platform.updatedAt)}`}
                  </span>
                </button>
              ))}
            </div>
            ...
            <EmptyState
              title="暂无平台状态"
              description="启动并配置消息平台后，这里会显示各平台连接状态。"
            />
          )}
          {platforms.length > visiblePlatforms.length ? (
            <p className="helper-text top-gap">
              其余 {hiddenPlatformCount} 个平台已先收起，避免默认把入口页铺成整面列表。
            </p>
          ) : null}
          <Toolbar className="top-gap">
            <Button kind="primary" onClick={() => navigate('logs', logsIntent)}>
              查看日志
            </Button>
            ...
            remoteJobs.length > 4 ? (
              <Toolbar>
                <Button onClick={() => setShowAllRemoteJobs(value => !value)}>
                  {showAllRemoteJobs
                    ? '收起作业'
                    : `展开更多作业（剩余 ${hiddenRemoteJobCount} 个）`}
                </Button>
              </Toolbar>
            ) : undefined
          }
        >
          <div className="workspace-summary-strip">
            <section className="summary-mini-card">
              ...
              <span className="summary-mini-meta">
                {remoteJobs.length > 0 ? '这里只看依赖 Gateway 的作业' : '当前仍以本地交付为主'}
              </span>
            </section>
            <section className="summary-mini-card">
              ...
              <span className="summary-mini-meta">
                {failingRemoteJobs[0]?.name || '当前没有交付失败'}
              </span>
            </section>
            <section className="summary-mini-card">
              <span className="summary-mini-label">下一次调度</span>
              <strong className="summary-mini-value">
                {formatTimestamp(prioritizedRemoteJobs[0]?.nextRunAt)}
              </strong>
              <span className="summary-mini-meta">
                {prioritizedRemoteJobs[0]?.name || '暂无远端调度'}
              </span>
            </section>
            <section className="summary-mini-card">
              <span className="summary-mini-label">建议去向</span>
              <strong className="summary-mini-value">
                {failingRemoteJobs.length > 0 ? '先看日志' : '继续看 Cron'}
              </strong>
              <span className="summary-mini-meta">
                {failingRemoteJobs.length > 0
                  ? '错误日志通常更快定位问题'
                  : '回到 Cron 继续核对 delivery'}
              </span>
            </section>
          </div>
          {visibleRemoteJobs.length ? (
                  ...
                  onClick={() =>
                    navigate(
                      ...
                      buildLogsDrilldownIntent(relaySeed, {
                        ...
                        logName:
                          job.lastDeliveryError || job.lastError ? 'gateway.error' : 'gateway',
                        contains: job.id,
                        limit: '160',
                      })
                    )
                ...
                >
                  ...
                  <span>
                    {job.lastDeliveryError ||
                      job.lastError ||
                      `下次 ${formatTimestamp(job.nextRunAt)}`}
                  </span>
                </button>
              ))}
            </div>
            ...
            <EmptyState
              title="暂无远端作业"
              description="当前 profile 还没有依赖 gateway 的 delivery 作业，或者所有作业都走本地交付。"
            />
          )}
          {remoteJobs.length > visibleRemoteJobs.length ? (
            <p className="helper-text top-gap">
              其余 {hiddenRemoteJobCount} 个作业已先收起，需要时再展开全部。
            </p>
          ) : null}
          <Toolbar className="top-gap">
            <Button kind="primary" onClick={() => navigate('cron')}>
              进入 Cron
            </Button>
        ...
        >
          <div className="workspace-summary-strip">
            <section className="summary-mini-card">
              ...
              <span className="summary-mini-meta">
                {gatewayContains ? `优先筛 ${gatewayContains}` : '直接看网关主日志'}
              </span>
            </section>
            <section className="summary-mini-card">
            ...
            <section className="summary-mini-card">
              ...
              <span className="summary-mini-meta">
                {failingRemoteJobs.length > 0
                  ? `${failingRemoteJobs.length} 个异常待核对`
                  : '当前没有明显交付失败'}
              </span>
            </section>
            <section className="summary-mini-card">
          ...
          <div className="workspace-shortcut-grid dashboard-launcher-grid top-gap">
            ...
            >
              <strong>
                <span className="dashboard-shortcut-icon">🧾</span>查看日志
              </strong>
              <span>
                {gatewayLogName === 'gateway.error' ? '优先看错误日志' : '先看总日志确认链路'}
              </span>
            </button>
            <button
              type="button"
            ...
            >
              <strong>
                <span className="dashboard-shortcut-icon">🧭</span>进入诊断
              </strong>
              <span>
                {warnings.length > 0 ? '带着异常平台和作业继续深检' : '当前可以直接做深检确认'}
              </span>
            </button>
            <button
              type="button"
            ...
            >
              <strong>
                <span className="dashboard-shortcut-icon">📤</span>查看 Cron
              </strong>
              <span>
                {remoteJobs.length > 0
                  ? '继续核对 delivery 调度和失败回执'
                  : '确认是否仍然只走本地交付'}
              </span>
            </button>
            <button
              type="button"
              className="workspace-shortcut-card dashboard-shortcut-card"
              onClick={() => navigate('config', configIntent)}
            >
              <strong>
                <span className="dashboard-shortcut-icon">⚙️</span>核对配置
              </strong>
  ...
  const repairSection = (
    <>
        ...
        >
          <div className="workspace-summary-strip">
            <section className="summary-mini-card">
              ...
              <strong className="summary-mini-value">
                {installation.binaryFound ? '已就绪' : '缺失'}
              </strong>
              <span className="summary-mini-meta">
                {installation.binaryFound ? '可以继续做 service 接管和诊断' : '请先安装 Hermes CLI'}
              </span>
            </section>
            <section className="summary-mini-card">
            ...
            <section className="summary-mini-card">
              ...
              <span className="summary-mini-meta">
                {gatewayRunning ? `PID ${gateway?.pid ?? '—'}` : '下一步建议先启动或重启'}
              </span>
            </section>
            <section className="summary-mini-card">
          ...
          <div className="control-card-grid top-gap">
            <section className="action-card action-card-compact">
              ...
              <p className="action-card-copy">
                客户端只做接管封装，不重复实现 Hermes 内部 service 逻辑。
              </p>
              <Toolbar>
                <Button
                  kind="primary"
                  onClick={() =>
                    void openInTerminal(
                      'gateway:install-service',
                      '安装 Gateway Service',
                      installation.gatewayInstallCommand
                    )
                  }
                  disabled={runningCommand !== null || !installation.binaryFound}
                >
                ...
                <Button
                  onClick={() =>
                    void openInTerminal(
                      'gateway:setup-service',
                      'Gateway Setup',
                      installation.gatewaySetupCommand
                    )
                  }
                  disabled={runningCommand !== null || !installation.binaryFound}
                >
                ...
                <Button
                  ...
                  onClick={() =>
                    void openInTerminal(
                      'gateway:uninstall-service',
                      '卸载 Gateway Service',
                      installation.gatewayUninstallCommand,
                      '确定卸载当前 profile 的 gateway service 吗？'
                    )
                  }
                  disabled={runningCommand !== null || !installation.binaryFound}
                >
                  {runningCommand === 'gateway:uninstall-service'
                    ? '卸载 Service…'
                    : '卸载 Service'}
                </Button>
              </Toolbar>
            ...
            <section className="action-card action-card-compact">
              ...
              <p className="action-card-copy">
                日常启停仍走 Hermes 原生命令，客户端只负责集中入口和结果回显。
              </p>
              <Toolbar>
                <Button
                  kind="primary"
        ...
        >
          <div className="workspace-summary-strip">
            ...
            <section className="summary-mini-card">
              ...
              <strong className="summary-mini-value">
                {installation.logsDirExists ? '已就绪' : '缺失'}
              </strong>
              <span className="summary-mini-meta">
                {installation.logsDirExists ? '可直接打开 logs 目录' : '当前还没有稳定日志材料'}
              </span>
            </section>
            <section className="summary-mini-card">
              <span className="summary-mini-label">远端依赖</span>
              <strong className="summary-mini-value">
                {remoteJobs.length ? `${remoteJobs.length} 个` : '暂无'}
              </strong>
              <span className="summary-mini-meta">
                {failingRemoteJobs.length > 0
                  ? `${failingRemoteJobs.length} 个作业异常`
          ...
          <div className="workspace-shortcut-grid dashboard-launcher-grid top-gap">
            ...
            >
              <strong>
                <span className="dashboard-shortcut-icon">🗂️</span>状态文件
              </strong>
              <span>
                {installation.gatewayStateExists
                  ? '直接定位 gateway_state.json'
                  : '状态文件尚未生成'}
              </span>
            </button>
            <button
            ...
            >
              <strong>
                <span className="dashboard-shortcut-icon">📁</span>日志目录
              </strong>
              <span>
                {installation.logsDirExists
                  ? '直接打开 Hermes logs 目录'
                  : '当前没有可打开的日志目录'}
              </span>
            </button>
            ...
            >
              <strong>
                <span className="dashboard-shortcut-icon">📤</span>Cron 编排
              </strong>
              <span>
                {remoteJobs.length > 0 ? '继续核对 delivery 作业调度' : '确认是否仍然只走本地链路'}
              </span>
            </button>
            <button
              type="button"
      ...
      {repairView === 'diagnostics' ? (
        <Panel title="诊断命令" subtitle="CLI 诊断命令单独放在这一层，默认不再占满修复页首屏。">
          <div className="workspace-summary-strip">
            <section className="summary-mini-card">
              <span className="summary-mini-label">最近命令</span>
              <strong className="summary-mini-value">{lastActionLabel ?? '暂无'}</strong>
              <span className="summary-mini-meta">
                {lastCommand?.cli || '进入此层后再执行具体诊断命令'}
              </span>
            </section>
            <section className="summary-mini-card">
              <span className="summary-mini-label">最近结果</span>
              <strong className="summary-mini-value">
                {diagnostic ? (diagnostic.success ? '成功' : '失败') : '未执行'}
              </strong>
              <span className="summary-mini-meta">
                {diagnostic ? `退出码 ${diagnostic.exitCode}` : '还没有新的 CLI 输出'}
              </span>
            </section>
            <section className="summary-mini-card">
              ...
              <strong className="summary-mini-value">{gatewayLogName}</strong>
              <span className="summary-mini-meta">
                {gatewayContains ? `诊断后优先筛 ${gatewayContains}` : '诊断后可直接回日志页'}
              </span>
            </section>
            <section className="summary-mini-card">
              <span className="summary-mini-label">下一步</span>
              <strong className="summary-mini-value">
                {warnings.length > 0 ? '先查异常' : '做基础体检'}
              </strong>
              <span className="summary-mini-meta">
                深诊断通常只在异常平台、失败作业或配置漂移时再进入。
              </span>
            </section>
          </div>
            ...
            {GATEWAY_DIAGNOSTIC_COMMANDS.map(item => (
              <section className="action-card" key={item.key}>
                ...
                <Toolbar>
                  ...
                  >
                    {runningCommand === `diagnostic:${item.key}`
                      ? `${item.label}…`
                      : `执行 ${item.label}`}
                  </Button>
                  <Button
                    onClick={() =>
                      navigate(
                        item.relatedPage,
                        item.relatedPage === 'logs'
                          ? logsIntent
                          : item.relatedPage === 'config'
                            ? configIntent
                            : undefined
                      )
                    }
          ...
          <Toolbar className="top-gap">
            <Button kind="primary" onClick={() => setRepairView('output')}>
              查看命令输出
            </Button>
          ...
          {diagnostic ? (
            <div className="result-stack">
              <div className="detail-list compact">
                <KeyValueRow
                  label="命令类型"
                  value={lastActionLabel ?? lastCommand?.label ?? '网关动作'}
                />
                ...
                <KeyValueRow
                  label="结果"
                  value={
                    <Pill tone={diagnostic.success ? 'good' : 'bad'}>
                      {diagnostic.success ? '成功' : '失败'}
                    </Pill>
                  }
                />
              </div>
            ...
            <EmptyState
              title="暂无诊断输出"
              description="点击任一网关动作或诊断命令后，这里会展示 Hermes 的原始输出。"
            />
          )}
          <Toolbar className="top-gap">
    ...
    <div className="page-stack">
      <div className="stat-cards">
        ...
        <StatCard
          ...
          meta={
            failingRemoteJobs.length > 0
              ? `${failingRemoteJobs.length} 个作业已有交付异常。`
              : '暂未发现远端投递失败。'
          }
      ...
      >
        <p className="helper-text">
          默认顺序：先看“常用总览”的链路判断，再去“平台与远端”核对平台或作业，最后才进入“修复与材料”处理
          service 接管和原始输出。
        </p>
      </Panel>
        ...
        <ContextBanner
          ...
          meta={
            <div className="pill-row">
              {investigation.platformName ? (
                <Pill tone="warn">{investigation.platformName}</Pill>
              ) : null}
      ...
      <div className="tab-bar">
        {GATEWAY_TABS.map(tab => (
          ...
          >
            {tab.label}
            {tab.key === 'delivery' &&
            (unhealthyPlatforms.length > 0 || failingRemoteJobs.length > 0) ? (
              <span className="tab-dirty-dot" />
            ) : null}
            {tab.key === 'repair' &&
            (warnings.length > 0 || (diagnostic && !diagnostic.success)) ? (
              <span className="tab-dirty-dot" />
            ) : null}
          </button>
        ))}
